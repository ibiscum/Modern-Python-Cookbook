...
class GIVEN_get_config_WHEN_load_THEN_overrides(unittest.TestCase):
    def setUp(self):
        self.mock_system_path = Mock(exists=Mock(return_value=True), open=mock_open())
        self.exist = Mock(exists=Mock(return_value=True), open=mock_open())
        self.not_exist = Mock(exists=Mock(return_value=False))
        ...
        self.mock_home_path = Mock(
            expanduser=Mock(return_value=self.mock_expanded_home_path)
        )
        self.mock_path = Mock(side_effect=[self.mock_system_path, self.mock_home_path])

        self.mock_load = Mock(side_effect=[{"some_setting": 1}, {"another_setting": 2}])

    def runTest(self):
        with patch("__main__.Path", self.mock_path), patch(
            "__main__.load_config_file", self.mock_load
        ):
            config = get_config()
        # print(config)
        ...
        self.mock_path.assert_has_calls([call("/etc/profile"), call("~")])
