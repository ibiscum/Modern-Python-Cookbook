...
specification = {
    "swagger": "2.0",
    "info": {"title": """Python Cookbook\nChapter 12, recipe 6.""", "version": "1.0"},
    "schemes": ["http"],
    "host": "127.0.0.1:5000",
    "basePath": "/dealer",
...
def make_player():
    ...
    response = make_response(jsonify(status="ok", id=id), 201)
...
def get_players():
    response = make_response(
        jsonify({k: v.to_json() for k, v in user_database.items()})
    )
...
def get_player(id):
    ...
    response = make_response(jsonify(user_database[id].to_json()))
    response.headers["Content-Type"] = "application/json;charset=utf-8"
    return response
